...
class FullShipmentsDatatable extends DataTableComponent
{
    use WireToast;
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Route AVG",'route_avg')
                ->format(fn($value, $row, Column $column) => view('datatable.routeDifference')->withValue(($row->GetAverageVoyageIndicatorAttribute() ?? null)))
                ->searchable()
                ->sortable()
                ->collapseOnMobile(),
            ...
            SelectFilter::make('Last Updated')
                ...
                ->filter(function(Builder $builder, string $value) {
                    if ($value === 'since last login') {
                        ...
                        $builder->whereBetween('shipments.updated_at', [now()->startOfWeek(), now()->end()]);
                    }
                }),
    ...
    public function exportSelected()
    {
        ...
        else
        {
            ...
            if(count($shipments))
            {
                ...
                return Excel::download(new CardClickExport($this->GetShipmentHeadersAndData($shipments)), 'ShipmentTableExport.xlsx');
            }
            else
            {
                toast()->warning('No records where selected from the table','Warning')->push();
