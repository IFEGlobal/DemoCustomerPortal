...
class FullContainersDatatable extends DataTableComponent
{
    use WireToast;
    ...
    public function configure(): void
    {
        ...
        $this->setDefaultSort('shipments.estimated_arrival', 'asc');
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Port of Discharge", 'shipment.disc_port_name')
                ...
                ->sortable()
                ->collapseOnMobile(),
            Column::make("Est Dept", 'shipment.estimated_departure')
                ...
                ->filter(function(Builder $builder, string $value) {
                    if ($value == 0) {
                        $builder->whereHas('containerDelivery', function ($limit) {
                            $limit->whereDate('arrival_estimated_delivery', today());
                        });
                    } elseif ($value == 1) {
                        $builder->whereHas('containerDelivery', function ($limit) {
                            $limit->whereBetween('arrival_estimated_delivery', [now()->startOfWeek(), now()->endOfWeek()]);
                        });
                    }elseif ($value == 2) {
                        $builder->whereHas('containerDelivery', function ($limit) {
                            $limit->whereBetween('arrival_estimated_delivery', [now()->startOfMonth(), now()->endOfMonth()]);;
                        });
                    }
                }),
    ...
    public function exportSelected()
    {
        ...
        else
        {
            ...
            if(count($containers))
            {
                ...
                return Excel::download(new CardClickExport($this->GetContainersHeadersAndData($containers)), 'ContainerTableExport.xlsx');
            }
            else
            {
                toast()->warning('No records where selected from the table','Warning')->push();
