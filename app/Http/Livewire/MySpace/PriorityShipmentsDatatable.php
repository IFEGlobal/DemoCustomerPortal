...
class PriorityShipmentsDatatable extends DataTableComponent
{
    use WireToast;
    ...
    {
        $priority = Priority::query()->where('user_id', auth()->user()->id)->where('prioratable_type', 'App\Models\Shipment')->get()->pluck('prioratable_reference')->toArray();

        return Shipment::whereIn('shipment_ref', $priority);
    }
    ...
    public function removeFromPriority()
    {
        ...
        if (empty($ids))
        {
            ...
            DB::table('priorities')->where('user_id', auth()->user()->id)->whereIn('prioratable_reference', $shipments)->delete();

            $this->emit('refreshComponent');

            toast()->info('Success', $shipments->count().' records have been removed from your list')->push();
        }
    ...
    public function addToFavorites()
    {
        ...
        if (empty($ids)) {
            ...
            foreach ($shipments as $shipment) {
                ...
                $addToPriority = new AddToFavorites($model, $shipment->id, Auth::user()->id, $shipment->shipment_ref);
    ...
    public function exportSelected()
    {
        ...
        else
        {
            ...
            if(count($shipments))
            {
                ...
                return Excel::download(new CardClickExport($this->GetShipmentHeadersAndData($shipments)), 'PriorityShipmentTableExport.xlsx');
            }
            else
            {
                toast()->warning('No records where selected from the table','Warning')->push();
