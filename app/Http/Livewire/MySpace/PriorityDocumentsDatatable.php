...
class PriorityDocumentsDatatable extends DataTableComponent
{
    use WireToast;
    ...
    {
        $priority = Priority::query()->where('user_id', auth()->user()->id)->where('prioratable_type', 'App\Models\Document')->get()->pluck('prioratable_reference')->toArray();

        return Document::whereIn('document_id', $priority);
    }
    ...
    public function removeFromPriority()
    {
        ...
        if (empty($ids))
        {
            ...
            DB::table('priorities')->where('user_id', auth()->user()->id)->whereIn('prioratable_reference', $documents)->delete();

            $this->emit('refreshComponent');

            toast()->info($documents->count().' documents have been removed from your priority list', 'Success')->push();
        }
