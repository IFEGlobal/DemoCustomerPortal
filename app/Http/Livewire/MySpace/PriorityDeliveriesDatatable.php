...
class PriorityDeliveriesDatatable extends DataTableComponent
{
    use WireToast;
    ...
    {
        $priority = Priority::query()->where('user_id', auth()->user()->id)->where('prioratable_type', 'App\Models\ContainerDelivery')->get()->pluck('prioratable_reference')->toArray();

        return ContainerDelivery::whereIn('container_id', $priority);
    }
    ...
    public function removeFromPriority()
    {
        ...
        if (empty($ids))
        {
            ...
            DB::table('priorities')->where('user_id', auth()->user()->id)->whereIn('prioratable_reference', $arrayIds)->delete();

            $this->emit('refreshComponent');

            toast()->info($deliveries->count().' deliveries have been removed from your priority list', 'Success')->push();
        }
