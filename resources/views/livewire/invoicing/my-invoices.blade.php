        ...
        <div class="mt-4 grid grid-cols-12 gap-4 px-[var(--margin-x)] transition-all duration-[.25s] sm:mt-5 sm:gap-5 lg:mt-6 lg:gap-6 w-full">
            ...
            <div class="lg:flex lg:items-center lg:justify-between col-span-12">
                <div class="min-w-0 flex-1">
                    <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl sm:tracking-tight">Pending Invoices</h2>
                        ...
                        <div class="relative">
                            <div wire:click="$emitTo('documents.full-documents-datatable', 'clearFilters')" class="flex cursor-pointer items-start rounded-xl bg-white p-4 shadow-lg dark:bg-gray-800">
                                <div class="flex h-12 w-12 items-center justify-center rounded-full bg-white">
                                    <img src="/icons/invoice.svg" width="80">
                                </div>

                                <div class="ml-4">
                                    <h2 class="font-semibold">{{ $AllInvoices ?? 0 }}</h2>
                                    <p class="mt-2 text-sm font-semibold text-gray-500 dark:text-white">All Invoices</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="relative group">
                            ...
                            <div class="relative">
                                <div wire:click="$emitTo('documents.full-documents-datatable', 'setFilter', 'updated', '2')" class="flex cursor-pointer items-start rounded-xl bg-white p-4 shadow-lg dark:bg-gray-800">
                                    <div class="flex h-12 w-12 items-center justify-center rounded-full bg-white">
                                        <img src="/icons/overdue.svg" width="80">
                                    </div>

                                    <div class="ml-4">
                                        <h2 class="font-semibold">{{ $OverdueInvoices ?? 0 }}</h2>
                                        <p class="mt-2 text-sm font-semibold text-gray-500 dark:text-white">Overdue Invoices</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="relative group">
                        ...
                        <div class="relative">
                            <div wire:click="$emitTo('documents.full-documents-datatable', 'setFilter', 'received', '2')" class="flex cursor-pointer items-start rounded-xl bg-white p-4 shadow-lg dark:bg-gray-800">
                                <div class="flex h-12 w-12 items-center justify-center rounded-full bg-white">
                                    <img src="/icons/7-days.svg" class="w-8 h-8">
                                </div>

                                <div class="ml-4">
                                    <h2 class="font-semibold">{{ $InvoiceDueThisWeek ?? 0 }}</h2>

                                    <p class="mt-2 text-sm font-semibold text-gray-500 dark:text-white">Due {{ now()->startOfWeek()->format('d') }} - {{ now()->endOfWeek()->format('d') }} of {{ now()->format('M y') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative group">
                        ...
                        <div class="relative">
                            <div wire:click="$emitTo('documents.full-documents-datatable', 'setFilter', 'document', '0')" class="flex cursor-pointer items-start rounded-xl bg-white p-4 shadow-lg dark:bg-gray-800">
                                ...
                                <div class="ml-4">
                                    <h2 class="font-semibold">{{ $InvoicesDueThisMonth ?? 00  }} </h2>

                                    <p class="mt-2 text-sm font-semibold text-gray-500 dark:text-white">Due {{ now()->startOfMonth()->format('d') }} - {{ now()->endOfMonth()->format('d') }} of {{ now()->format('M y') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card bg-white h-full p-4 dark:bg-gray-800 shadow-lg">
                    ...
                    <div class="mt-2">
                        <p class="text-xs+ text-center text-gray-500">
                            This <strong>Table</strong> is limited to outstanding invoices.
                        </p>
                    </div>
                    <div class="mt-2">
                        <p class="text-xs+ text-center text-gray-500">
                            You're <strong>Invoices</strong> are also avaliable in <strong>Documents</strong>.
                        </p>
                    </div>
                    <div class="mt-2">
                        <p class="text-xs+ text-center text-gray-500">
            ...
            <div class="w-full h-full bg-white shadow-lg rounded-xl overflow-hidden dark:bg-gray-800 p-4 mb-1 col-span-12">
                <div class="flex flex-wrap mb-4 p-2">
                    <div class="flex items-center">
                        <span class="self-center text-xl font-semibold whitespace-nowrap dark:text-white">My Invoices</span>
                    </div>
                </div>
                <livewire:invoicing.job-invoicing-datatable/>
            </div>
        </div>
    </main>
</div>
